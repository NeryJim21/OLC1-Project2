...
export class ToUpper extends Expression {
    ...
    public run(globalST:SymbolTable, localST:SymbolTable, method:MethodTable, environment:string):Value{
        ...
        if(value.type !== ValueType.STRING){
            output.setOutput(`-->Semántico, TOUPPER no es aplicable a tipo: ${ValueType[value.type]} (${this.line}:${this.column}).`)
            throw new Error("Semántico", `TOUPPER no es aplicable a tipo: ${ValueType[value.type]}.`, this.line, this.column)
        }
        return {type: ValueType.STRING, value: ((value.value).toUpperCase())}
    }
