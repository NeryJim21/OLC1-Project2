...
export class Round extends Expression {
    ...
    public run(globalST:SymbolTable, localST:SymbolTable, method:MethodTable, environment:string):Value{
        ...
        if(value.type !== ValueType.INT && value.type !== ValueType.DOUBLE){
            output.setOutput(`-->Semántico, ROUND no es aplicable a tipo: ${ValueType[value.type]} (${this.line}:${+this.column}).`)
            throw new Error("Semántico", `ROUND no es aplicable a tipo: ${ValueType[value.type]}.`, this.line, this.column)
        }
        return {type: ValueType.INT, value: (Math.round(value.value))}
    }
