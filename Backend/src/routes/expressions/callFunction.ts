...
export class CallFunction extends Expression{
    ...
    public run(globalST:SymbolTable, localST:SymbolTable, methods:MethodTable, environment:string):Value{
                ...
                if(method.parameters.length === this.atributes.length){
                    ...
                    if(control != null && control != undefined){
                        if(control.type == ControlType.RETURN){
                            ...
                            this.setError(`No se puede retornar tipo ${ValueType[control.value.type]} en método: ${ValueType[method.type]}`, control.line, control.column)
                        }
                    }
                }
        ...
        throw new Error(`Semántico`, `No existe la función: ${this.id}`, this.line, this.column)
    }
